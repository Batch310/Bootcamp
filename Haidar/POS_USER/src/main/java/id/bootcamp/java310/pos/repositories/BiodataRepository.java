...
public interface BiodataRepository extends JpaRepository<BiodataEntity, Long> {
	...
	@Query(nativeQuery = true, value = "update biodata\r\n"
			...
			+ "where id = :biodata_id ")
	public void uploadPicture(@Param("image_path") String imagePath,
			@Param("biodata_id") Long biodataId);
	
	@Query(nativeQuery = true, value = "select biodata_id "
			+ "from m_user "
			+ "where id = :user_id")
	public Long getBiodataIdByUserId(@Param("user_id") Long userId);
	...
	@Query(nativeQuery = true, value = "update biodata\r\n"
			...
			+ "where id = :biodata_id")
	public void updateNamePhone(@Param("name") String name,
