...
public interface VariantRepository extends JpaRepository<VariantEntity, Long> {
	...
	// Create
	@Query(nativeQuery = true, name = "get_variant")
	public List<VariantDTO> getAllVariant();

	// Pagination
	@Query(nativeQuery = true, name = "pagination_variant")
	public List<VariantDTO> getPagination(@Param("keyword") String keyword, @Param("limit") int limit,
	...
	@Query(nativeQuery = true, value = "update variant\r\n" 
			...
			+ "is_delete = true " 
			+ "where id = :#{#dto.id}")
	public void updateKhususVariant(
			@Param("dto") VariantDTO dto, 
	...
	public void deleteVariant(@Param("id") Long id);
	...
	// Hitung Total Data
	@Query(nativeQuery = true, value = "select count(*) from variant where is_delete = false and name ilike '%' || :keyword ||'%'")
