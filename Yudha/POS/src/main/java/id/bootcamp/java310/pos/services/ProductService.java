...
public class ProductService {
	...
	public List<ProductDTO> getAll() {
		List<Product> listSumber = pr.findAll();
		List<ProductDTO> listHasil = new ArrayList<>();

		for (Product pSumber : listSumber) {
			ProductDTO pHasil = new ProductDTO(pSumber.getVariant().getCategory().getName(),
					pSumber.getVariant().getName(), pSumber.getInitial(), pSumber.getName(),
					pSumber.getDescription(), pSumber.getPrice(), pSumber.getStock(), pSumber.getActive(),
					pSumber.getVariant().getCategoryId(), pSumber.getVariantId(), pSumber.getId());
	...
	public Long insertItem(ProductDTO dto) throws Exception {
		...
		Product pEntity = new Product(dto.getId(), dto.getVariant_id(), dto.getInitial(), dto.getName(),
				dto.getDescription(), dto.getPrice(), dto.getStock(), dto.getActive(), dto.getCreate_by(), new Date());

		Product hasil = pr.save(pEntity);
		return hasil.getId();
	}
	...
	public List<ProductDTO> searchProduct(String keyword) {
		List<Product> listSumber = pr.searchProducts(keyword);
		List<ProductDTO> listHasil = new ArrayList<>();
