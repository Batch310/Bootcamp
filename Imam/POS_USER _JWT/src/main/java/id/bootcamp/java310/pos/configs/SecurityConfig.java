...
public class SecurityConfig {
	...
	@Bean
	public DaoAuthenticationProvider authenDaoAuthenticationProvider() {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.cors().and().csrf().disable().exceptionHandling().authenticationEntryPoint(authEntryPointJwt).and()
				...
				.hasAnyRole("ADMIN", "CASHIER").anyRequest().authenticated() // Setelah permit(izin) all, harus auth
				.and().httpBasic() // Auth gagal, yg ditampilkan apa? basicnya
				.and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
		// No cache/Session di API
		
		http.authenticationProvider(authenDaoAuthenticationProvider());
